...
/////////////////////////////////////////////////////////////////////
  ...
  */
  ...
  public:
    ...
    Vector() : _begin(0), _end(0) {} ;
    Vector(index_type sz) { allocate(sz) ; } ;
    virtual ~Vector(void) { free() ; }

    void new_dim(index_type sz)
    { if ( _size != sz ) { free() ; allocate(sz) ; } }
  ...
  namespace sparselib_fun {
    ...
    GeneratePlaneRotation(T const & dx, T const & dy, T & cs, T & sn) {
      ...
      if ( dy == 0.0 ) {
        cs = 1.0;
        sn = 0.0;
      } else if ( ::sparselib_fun::absval(dy) > ::sparselib_fun::absval(dx) ) {
        T temp = dx / dy;
        sn = 1.0 / ::sparselib_fun::sqrt( 1.0 + temp*temp );
        cs = temp * sn;
      } else {
        T temp = dy / dx;
        cs = 1.0 / ::sparselib_fun::sqrt( 1.0 + temp*temp );
        sn = temp * cs;
      }
    }
